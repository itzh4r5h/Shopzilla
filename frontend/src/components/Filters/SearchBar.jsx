...
import { saveKeyword as serachAdminProduct } from "../../store/slices/admin/adminProductSlice";
import { setKeyword } from "../../store/slices/admin/variantSlice";
...
export const SearchBar = ({ placeholderValue, path }) => {
  ...
  const sendToProductsPage = (e) => {
    if (e.key === "Enter") {
      if (search.trim().length > 0 && path === "/products") {
        dispatch(serachProduct(search.trim()));
        navigate(path);
      } else if (path === "/admin/dashboard/products/all") {
        dispatch(serachAdminProduct(search.trim()));
        navigate(path);
      } else if (path === "/admin/dashboard/products/out_of_stock") {
        dispatch(setKeyword(search.trim()));
        navigate(path);
      } else if (path === "/admin/dashboard/users") {
